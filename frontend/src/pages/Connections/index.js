...
import ZapModal from "../../components/ZapModal.js"; // Importe o novo componente
...
const IconChannel = (channel) => {
  switch (channel) {
    case "telegram":
      return <Telegram style={{ color: "#3b5998" }} />;
    case "facebook":
      return <Facebook style={{ color: "#3b5998" }} />;
      case "wppoficial":
      return <WhatsApp style={{ color: "#3b5998" }} />;
    case "instagram":
...
const Connections = () => {
  ...
  const [zapModalOpen, setZapModalOpen] = useState(false); // Estado para o novo modal
  ...
  const handleCloseZapModal = useCallback(() => {
    setZapModalOpen(false);
  }, [setZapModalOpen]);

  const handleSaveZap = (data) => {
    // Aqui você pode enviar os dados para o backend ou fazer outra ação necessária
    console.log("Dados do Zap:", data);
  };
  ...
  return (
    <MainContainer>
      ...
      <ZapModal
        open={zapModalOpen}
        onClose={handleCloseZapModal}
        onSave={handleSaveZap}
      />
      <MainHeader>
