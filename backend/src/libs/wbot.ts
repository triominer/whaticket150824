...
export type Session = WASocket & { id?: number; store?: Store };
...
type idZap = { id: number; whatsappId: number };
const idZapArray: idZap[] = []; // Inicializa idZapArray como um array de objetos idZap

export const getWbotWpp = async (whatsappId: number): Promise<Whatsapp> => {
  console.log("Procurando pelo whatsappId:", whatsappId);
  const zapEntry = await Whatsapp.findOne({ where: { id: whatsappId } });
  if (!zapEntry) {
    throw new AppError("getWbotWpp - não capturou idZap!", whatsappId);
  }
  return zapEntry;
};
