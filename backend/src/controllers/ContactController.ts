...
export const createContact = async (req: Request, res: Response): Promise<Response> => {
  const { name, number, email, isGroup, companyId } = req.body;
 // const { companyId } = req.user;

 /* if (!companyId) {
    throw new AppError("companyId is missing", 400);
  }*/
  ...
  try {
    await schema.validate({ name, number, email });

    const cleanedNumber = number.replace(/[- )(._+]/g, '');
