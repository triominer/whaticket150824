...
import sendFaceMessage, { sendFacebookMessage } from "../services/FacebookServices/sendFacebookMessage";
...
import { sendFacebookMessageFileExternal, sendFacebookMessageMedia, sendFacebookMessageMediaExternal } from "../services/FacebookServices/sendFacebookMessageMedia";
import { sendMessageToTelegramChat } from "../services/Telegram/Telegram";
...
export const store = async (req, res) => {
  ...
  if (ticket.channel === "whatsapp" || ticket.channel === "wppoficial") {
      SetTicketMessagesAsRead(ticket);
  }
  try {
      if (medias) {
          await Promise.all(
              medias.map(async (media) => {
                  ...
                  if (ticket.channel === "wppoficial") {
                      await sendFacebookMessageMedia({ media, ticket });
                  }
                  if (ticket.channel === "telegram") {
                      //await sendTelegramMedia({ media, ticket });
                  }
              })
          );
      } else {
          ...
          if(ticket.channel === "telegram") {
            await sendMessageToTelegramChat(ticket.contact.number, body);
          }
      }
      return res.send();
  } catch (error) {
