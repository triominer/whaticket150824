...
type FindParams = { companyId: number; contactListId: number };
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const io = getIO();
  io.emit(`company-${companyId}-ContactListItem`, { action: "create", record });
...
export const update = async (
  ...
  const io = getIO();
  io.emit(`company-${companyId}-ContactListItem`, { action: "update", record });
  return res.status(200).json(record);
};
export const remove = async (
  ...
  const io = getIO();
  io.emit(`company-${companyId}-ContactListItem`, { action: "delete", id });
