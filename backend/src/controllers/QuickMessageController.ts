...
type FindParams = { companyId: string; userId: string };
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const io = getIO();
  io.emit(`company-${companyId}-quickmessage`, { action: "create", record });
...
export const update = async (
  ...
  const { id } = req.params;
  const record = await UpdateService({ ...data, userId: req.user.id, id });
  const io = getIO();
  io.emit(`company-${companyId}-quickmessage`, { action: "update", record });
  return res.status(200).json(record);
};
export const remove = async (
  ...
  const io = getIO();
  io.emit(`company-${companyId}-quickmessage`, { action: "delete", id });
