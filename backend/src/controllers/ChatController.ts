...
type StoreData = { users: any[]; title: string };
type FindParams = { companyId: number; ownerId?: number };
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const record = await CreateService({ ...data, ownerId, companyId });
  const io = getIO();
...
export const update = async (
  ...
  const data = req.body;
  const { id } = req.params;
  const record = await UpdateService({ ...data, id: +id });
  const io = getIO();
  record.users.forEach(user => {
    io.emit(`company-${companyId}-chat-user-${user.userId}`, {
      action: "update",
...
export const saveMessage = async (
  ...
  const chatId = +id;
  const newMessage = await CreateMessageService({ chatId, senderId, message });
...
export const checkAsRead = async (
  ...
  const io = getIO();
  io.emit(`company-${companyId}-chat-${id}`, { action: "update", chat });
  io.emit(`company-${companyId}-chat`, { action: "update", chat });
