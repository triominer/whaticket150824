...
type StoreData = { name: string; companyId: string };
type FindParams = { companyId: string };
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const io = getIO();
  io.emit(`company-${companyId}-ContactList`, { action: "create", record });
...
export const update = async (
  ...
  const io = getIO();
  io.emit(`company-${companyId}-ContactList`, { action: "update", record });
  return res.status(200).json(record);
};
export const remove = async (
  ...
  const io = getIO();
  io.emit(`company-${companyId}-ContactList`, { action: "delete", id });
